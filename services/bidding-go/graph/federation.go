...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Auction":
		resolverName, err := entityResolverNameForAuction(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Auction": %w`, err)
		}
		switch resolverName {
		...
		case "findAuctionByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findAuctionByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindAuctionByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Auction": %w`, err)
			}
...
func entityResolverNameForAuction(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Auction", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Auction", ErrTypeNotFound))
			break
		}
		return "findAuctionByID", nil
	}
	return "", fmt.Errorf("%w for Auction due to %v", ErrTypeNotFound,
